...
<html lang="${req.locale_name}">
  <head>
    ...
    <link rel="stylesheet" href="${util.hashed_asset_url('reader.css')}">
  ...
  <body id="carrell">
    <div class="wrapper sidebar-container">
      ## sidebar
      % if cookie.get('carrell.sidebar', '') == 'locked':
      ...
      % endif
      <%block name='sidebar'>
        <%include file='aarau:templates/carrell/_sidebar.mako'/>
      </%block>
    ...
    <script src="${util.hashed_asset_url('reader.js')}" charset="utf-8" async></script>
    <%block name='script'>
    </%block>
  </body>
</html>
